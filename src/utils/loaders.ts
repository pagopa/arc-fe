...
const getPaymentNotices = () =>
  useQuery({
    ...
    queryFn: async () => {
      //this is wrong on purpose, to generate an error.
      const { data: transaction } = await utils.apiClient.transactions.getTransactionDetails();
      console.log(transaction);
      utils.zodSchema.transactionDTOSchema.parse(transaction);
      return [
        {
          payee: {
